...
const EditChannelModal = (props) => {
  ...
  const [conversationLogCaptureEnabled, setConversationLogCaptureEnabled] =
    useState(true);
  ...
  const loadChannel = async (globalConversationLogCaptureEnabled = true) => {
    setLoading(true);
    ...
    if (success) {
      ...
      if (!globalConversationLogCaptureEnabled) {
        data.conversation_log_enabled = false;
      }
  ...
  const fetchConversationLogCaptureEnabled = async () => {
    if (!isRoot()) {
      return false;
    }
    try {
      ...
      const enabled = res?.data?.data?.settings?.capture_enabled !== false;
      setConversationLogCaptureEnabled(enabled);
      if (!enabled) {
        setInputs((prev) => ({ ...prev, conversation_log_enabled: false }));
      ...
      setConversationLogCaptureEnabled(true);
      return true;
    }
  };
  ...
  useEffect(() => {
    ...
    if (props.visible) {
      void (async () => {
        const globalConversationLogCaptureEnabled =
          await fetchConversationLogCaptureEnabled();
        if (isEdit) {
          await loadChannel(globalConversationLogCaptureEnabled);
        } else {
          const initValues = {
            ...getInitValues(),
            conversation_log_enabled: false,
            ...
            const advancedSettingsContent = (
              <div className='space-y-4'>
                ...
                <div className='pt-3'>
                  ...
                  {isRoot() && (
                    <Form.Switch
                      field='conversation_log_enabled'
                      label={t('会话日志采集')}
                      checkedText={t('开')}
                      uncheckedText={t('关')}
                      disabled={!conversationLogCaptureEnabled}
                      onChange={(value) => {
                        if (!conversationLogCaptureEnabled) return;
                        handleChannelOtherSettingsChange(
                      ...
                      extraText={
                        conversationLogCaptureEnabled
                          ? t('保存完整对话 payload 用于 strict traj 导出')
                          : t('全局会话日志采集已关闭，渠道开关不会生效')
                      }
                    />
                  )}
